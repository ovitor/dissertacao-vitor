...
\figurasimples{arquitetura}{Visão geral arquitetural da solução}{12cm}
...
% Para habilitar a recepção do sinal digital nas televisões, a utilização do 
% \stb[] externo ou um receptor digital interno à TV se torna imprescindível.
...
\subsection{\textit{Software} implementado}\label{sec:software-implementado}
\label{subsec:aplicacoes-tv-health}
...
O módulo \textit{service.panicbutton} 

\section{Visão de Engenharia} \label{sec:visao-engenharia}

incluir figura visao de engenharia (nazim)
descrever detalhes tecnicos
